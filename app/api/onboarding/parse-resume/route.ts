...
function sniff(buf: Buffer): "pdf" | "docx" | "unknown" {
  if (!buf || !Buffer.isBuffer(buf) || buf.length < 4) return "unknown";
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      throw new HttpError(400, "We couldn't read text from that file — it may be corrupted or password-protected. Try pasting your resume instead.");
    }
    ...
    if (text.length < 20) {
      throw new HttpError(400, "We couldn't read enough text from that file — try pasting your resume instead.");
    }
